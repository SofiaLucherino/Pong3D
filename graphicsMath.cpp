...
i32_x4 I32X4(i32 A, i32 B, i32 C, i32 D)
{
	i32_x4 Result = {};
	Result.Vals = _mm_set_epi32(D, C, B, A);
...
void F32X4Store(f32* Ptr, f32_x4 Value)
{
	_mm_storeu_ps(Ptr, Value.Vals);
}

f32_x4 F32X4ReInterpret(i32_x4 A)
{
	f32_x4 Result = {};
	Result.Vals = _mm_castsi128_ps(A.Vals);
...
void I32X4Store(i32* Ptr, i32_x4 Value)
{
	_mm_storeu_si128((__m128i*)Ptr, Value.Vals);
}

i32_x4 I32X4ReInterpret(f32_x4 A)
{
	i32_x4 Result = {};
	Result.Vals = _mm_castps_si128(A.Vals);
...
v2i_x4 V2IX4(i32_x4 A, i32_x4 B)
{
	v2i_x4 Result = {};
	Result.x = A;
	Result.y = B;
...
v2_x4 operator*(f32_x4 A, v2_x4 B)
{
	v2_x4 Result = {};
	Result.x = B.x * A;
	Result.y = B.y * A;
...
v2i_x4 operator+(v2i_x4 A, v2i_x4 B)
{
	v2i_x4 Result = {};
	Result.x = A.x + B.x;
	Result.y = A.y + B.y;
...
m4 PerspectiveMatrix(f32 Fov, f32 AspectRation, f32 NearZ, f32 FarZ)
{
	//Ми очікуємо що Fov є у градусах
	m4 Result = {};
